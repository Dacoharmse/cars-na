...
export default function Header() {
  ...
  return (
    <>
      ...
      {/* Utility Bar */}
      <div className={clsx(
        "fixed top-0 inset-x-0 z-[60] border-b transition-all duration-300",
        isHomepage && !scrolled
          ? "bg-white/[0.04] backdrop-blur-sm border-white/[0.06]"
          : "bg-gradient-to-r from-slate-100 to-slate-50 dark:from-slate-800 dark:to-slate-900 border-slate-200 dark:border-slate-700"
      )}>
        <div className="mx-auto max-w-7xl px-4 h-10 flex items-center justify-between">
          ...
          <div className="flex md:hidden">
            <Link
              ...
              className={clsx(
                "flex items-center gap-2 text-xs font-semibold transition-colors group",
                isHomepage && !scrolled
                  ? "text-white/60 hover:text-white"
                  : "text-slate-700 dark:text-slate-300 hover:text-[#1F3469] dark:hover:text-white"
              )}
            >
          ...
          <div className="flex items-center gap-2 md:gap-8 ml-auto">
            ...
            <Link
              href="tel:+264-61-000-000"
              className={clsx(
                "hidden md:flex items-center gap-2 text-xs font-semibold transition-colors group",
                isHomepage && !scrolled
                  ? "text-white/60 hover:text-white"
                  : "text-slate-700 dark:text-slate-300 hover:text-[#1F3469] dark:hover:text-white"
              )}
            ...
            <div className={clsx(
              ...
              isHomepage && !scrolled ? "bg-white/15" : "bg-slate-300 dark:bg-slate-600"
            )}></div>
            ...
            {session ? (
              ...
              <>
                <Link
                  href="/dealer/login"
                  className={clsx(
                    "flex items-center gap-1 md:gap-2 text-xs font-semibold transition-colors group",
                    isHomepage && !scrolled
                      ? "text-white/60 hover:text-white"
                      : "text-slate-700 dark:text-slate-300 hover:text-[#1F3469] dark:hover:text-white"
                  )}
      ...
      <header
        className={clsx(
          ...
          scrolled
            ...
            : isHomepage
              ? "top-10 h-16 bg-transparent border-white/[0.06]"
        ...
        <div className="mx-auto max-w-7xl h-full flex items-center justify-between px-4">
              ...
              {navLinks.map(({ href, label }) => (
                <li key={href}>
                  <Link
                    href={href}
                    className={clsx(
                      "relative text-sm font-semibold transition-colors group py-2",
                      isHomepage && !scrolled
                        ? "text-white/70 hover:text-white"
                        : "text-slate-700 dark:text-slate-100 hover:text-[#1F3469] dark:hover:text-white"
                    )}
                    aria-current={pathname === href ? 'page' : undefined}
                  >
                    {label}
                    <span className={clsx(
                      "absolute bottom-0 left-0 w-0 h-0.5 group-hover:w-full transition-all duration-200",
                      isHomepage && !scrolled ? "bg-[#C9A84C]" : "bg-[#1F3469] dark:bg-white"
                    )}></span>
                  </Link>
                </li>
              ))}
          ...
          <div className="flex items-center gap-4">
            ...
            <button
              ...
              className={clsx(
                "hidden sm:flex items-center gap-2 text-sm px-4 py-2 rounded-lg border transition-all duration-200 shadow-sm hover:shadow-md focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2",
                isHomepage && !scrolled
                  ? "text-white/70 hover:text-white border-white/15 bg-white/[0.06] hover:bg-white/[0.1]"
                  : "text-slate-600 dark:text-slate-300 hover:text-[#1F3469] dark:hover:text-white border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700"
              )}
              style={{ height: 44 }}
            >
            ...
            <button
              ...
              className={clsx(
                "lg:hidden p-2 rounded-lg transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2",
                isHomepage && !scrolled
                  ? "text-white/70 hover:text-white hover:bg-white/10"
                  : "text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 hover:text-[#1F3469] dark:hover:text-white"
              )}
